...
SRC 梁 せん断は、鉄骨部と RC 部を弾性分担（または地震時短期の構造規定方式 sQD/rQD）と比較する。
...
SRC 柱 せん断の長期は、併用式を全せん断力と比較する。

\\[ Q_A = (1+\beta) \cdot b \cdot {}\_r j \cdot a' \cdot f_s \\]

ここで \\( a' = {}\_r \alpha \\)（\\( b'/b \ge {}\_r \alpha/3 \\)）または \\( 3b'/b \\)、\\( 1 \le {}\_r \alpha \le 2 \\)、β は鉄骨ウェブの形式と寸法による係数とする。

短期は鉄骨部 `sQA`（強軸 \\( d_w \cdot t_w \cdot {}\_s f_s \\)／弱軸 \\( (4/3) \cdot b_f \cdot t_f \cdot {}\_s f_s \\)）と RC 部を分担 sQD/rQD と比較する。
...
ここで cMy には CFT 指針（コンクリート充填鋼管構造設計指針）の N-M 相互作用による終局曲げ耐力 Mu(N)（柱分類対応）を用い、柱頭・柱脚同一断面の仮定で \\( \sum {}\_c M_y = 2 \cdot M_u(N) \\) とする。
