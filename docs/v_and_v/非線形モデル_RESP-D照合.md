...
- ファイバー要素の鉄筋分離（現状は均質コンクリート断面。鉄筋・鉄骨を独立ファイバーと
  してモデル化していない）・第1折点 1.1倍/剛性低下率 α=1/1000
- SRC 梁のせん断終局（SRC規準/診断式/技術基準の 3 式・非充腹 格子/ラチス）
- 免震支承材（マルチシアスプリング・摩擦ばね。要素種別が未定義）
...
> 材端バネの曲げ終局強度化、NewRC コンクリート構成則（Fc≤60）。
