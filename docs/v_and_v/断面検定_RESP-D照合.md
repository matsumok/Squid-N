...
| B8 | 短期設計用せん断力 QD=min(QD1,QD2) の割増 n | ✅ 実装済み（RC梁・柱。地震時組合せで長期(G+P)結果から QE を分解、`n=1.5`、梁 QD1=QL+ΣBMy/l′・柱 QD1=ΣcMy/h′。SRC/CFT は弾性分担のまま＝残課題） | `rc.rs`（`seismic_design_shear`）, `lib.rs`（`SeismicQd`/`QdMethod`）, `rc_capacity.rs`（`rc_column_mu_simple`）, `app.rs`（`run_design_check` 配線） |
| B9 | PCa 水平接合面の検討 | 🔶 純関数実装（使用限界 τxy=Q・Sy/(b・I)・終局 τxy=ΔT/(b・Δl) と τu、M=0 位置の 2 次曲線式）。PCa 属性のデータモデルが無いためモデル自動配線は未実装 | `pca.rs`（新規） |
| B10 | SRC耐震壁・SRCパネルゾーン | ✅ 実装済み（耐震壁: 側柱 Qc に sfs・As を加算。パネル: cVe・jδ・fs・(1+β) 検定＋配線。jδ=3/2/2/1 の解釈は要原典再照合） | `joint.rs`（`rc_wall_shear_check`/`src_panel_zone_check`）, `joint_wiring.rs` |
...
| B13 | 多雪区域 δ1/δ2/δ3 の直接入力（設定化） | ✅ 実装済み（`SnowFactors`、既定 0.7/0.35/0.35。組合せ名・長短期判定も係数一般化。UI 入力は未対応＝API/データレベル） | `combo.rs`（`SnowFactors`/`standard_combinations`） |
