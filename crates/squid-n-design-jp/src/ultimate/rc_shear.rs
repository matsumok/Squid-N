...
//! **付着割裂による終局せん断耐力**（RESP-D マニュアル「計算編 06 終局検定」
//! 鉄筋コンクリート造梁／柱の終局耐力 b)）。
//!
//! # 位置付け
...
//! 塑性理論式（トラス機構＋アーチ機構）とは定式が異なる別物である。RESP-D では
...
//! - 軽量コンクリートのせん断終局耐力 0.9 倍低減: RESP-D「06 終局検定」共通事項。

/// コンクリート圧縮強度の有効係数 `ν0`（降伏ヒンジ・潜在ヒンジを計画しない時）。
///
/// `ν0 = 0.7 − Fc/200`（RESP-D「06 終局検定」）。`Fc` は [N/mm²]。
...
/// トラス機構の圧縮束の角度 φ のコタンジェント `cotφ`（RESP-D「06 終局検定」）。
///
/// ```text
...
/// アーチ機構の係数 `k1 = (√((L/D)² + 1) − (L/D)) / 2`（RESP-D「06 終局検定」）。
///
...
/// （RESP-D「06 終局検定」）。上限 1.0 でクランプする（`k2 ≤ 1.0`）。
///
...
/// 塑性理論式による終局せん断強度 `Qsu` の算定入力（RESP-D「06 終局検定」）。
...
/// 付着割裂による終局せん断耐力 `Qbu` の算定入力（RESP-D「06 終局検定」）。
#[derive(Clone, Copy, Debug)]
...
/// 付着割裂によって決定する終局せん断耐力 `Qbu` [N]（RESP-D「06 終局検定」）。
///
/// ```text
...
/// 割裂線長さ比 bi = min(bvi, bci, bsi)（RESP-D「06 終局検定」）。
///
/// ```text
/// bvi = √3·(2·Cmin/db1 + 1)
/// bci = √2·((Cs + Cb)/db1 − 1)
...
/// 異径鉄筋の 1 段目主筋の付着信頼強度 τbu [N/mm²]（RESP-D「06 終局検定」）。
///
/// ```text
