...
//! （RESP-D マニュアル「計算編 06 終局検定」鉄筋コンクリート造柱の終局耐力 a)）。
//!
//! # 位置付け
//! RESP-D は柱の終局曲げ強度 Mu を構造規定式（at 式、
...
/// 終局ひずみ εcu = 0.3%（RESP-D「06 終局検定」ACI）。
...
/// 等価応力度ブロックの深さ係数 `β1`（ACI318、RESP-D「06 終局検定」）。
///
/// ```text
/// β1 = { 0.85                         (Fc ≤ 4000 psi)
...
/// ACI 平面保持解析による柱の終局曲げ強度 `Mu` [N·mm]（RESP-D「06 終局検定」）。
///
