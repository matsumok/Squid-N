...
- **柱以外（梁・ブレース）の座屈長さ自動算定は未対応**: 軸別 K の自動算定は
  柱（`kind == MemberKind::Column`）にのみ適用され、梁・ブレースは直接入力
  （`lk_y_direct`/`lk_z_direct`）が無ければ部材長のままとなる。梁・ブレースの
  座屈長さ自動算定（例: ブレースの節点間距離に基づく算定）は本改修のスコープ外。
- **CFT 柱の軸別 K は未対応**: `steel_column_k_axes_with_index` は S 造柱
  （`cft/mod.rs` は独自の λ 算定経路）のみに配線されており、CFT 柱の座屈長さ
  自動算定は引き続き部材長基準（`DesignCtx.lk_y`/`lk_z` の直接入力は有効）。
- **梁の結合状態・支点の状態・剛域補正は引き続き考慮しない**: 軸別評価でも
  角度以外の簡略化（`EndCondition::SemiRigid` をピンとみなさない、剛域補正
  なし）は変更していない（モジュール doc「両版に共通する簡略化」参照）。
